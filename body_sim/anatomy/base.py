...
class AnatomicalComponent(BaseComponent, FluidContainer):
    ...
    def __init__(
        ...
        max_volume_multiplier: float = 1.5,
        ...
        if max_volume is not None:
            ...
            self._max_volume_multiplier = (
                max_volume / base_volume if base_volume > 0 else 1.5
            )
        else:
        ...
        if actual > 0:
            ...
            self._pending_events.append(
                (
                    ...
                    {"type": fluid_type.value, "amount": actual, "overflow": overflow},
                )
            )

        return actual

    def remove_fluid_by_type(
        self, amount: float, fluid_type: Optional[FluidType] = None
    ) -> float:
        """Удалить жидкость."""
        ...
        if total > 0:
            ...
            self._pending_events.append(
                (
                    EventType.FLUID_REMOVED,
                    {"amount": total, "types": [f.fluid_type.value for f in removed]},
                )
            )
        return total

    def stimulate(
        self, amount: float, source: str = None, stimulation_type: str = "general"
    ):
        """Стимулировать компонент."""
        self.current_stimulation = min(
            100.0, self.current_stimulation + amount * self.sensitivity
        )
        ...
        return {"pleasure": pleasure_gain, "pain": pain_gain, "arousal": amount * 0.01}
