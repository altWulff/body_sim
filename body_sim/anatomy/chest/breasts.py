...
class Breasts(AnatomicalComponent):
    ...
    def add_row(
        self, side: str = "aux", cup_size: CupSize = None, size_factor: float = 1.0
    ) -> BreastRow:
        """Добавить ряд груди."""
        ...
        row = BreastRow(index=len(self.rows), side=side, cup_size=cs)
    ...
    def stimulate(self, side: str, intensity: float = 1.0) -> Optional[dict]:
        ...
        if self._event_bus:
            event = Event(
                ...
                data={"intensity": intensity, "action": "stimulate", "result": result},
            )
    ...
    def express(
        self, side: Optional[str] = None, amount: float = None, pressure: float = 1.0
    ) -> float:
        """Сцеживание."""
        total = 0.0
        ...
        if side:
            ...
            if breast:
                amt = (
                    amount or breast.get_total_milk()
                    if hasattr(breast, "get_total_milk")
                    else amount or 100
                )
            ...
            for row in self.rows:
                amt = (
                    amount or row.get_total_milk()
                    if hasattr(row, "get_total_milk")
                    else amount or 100
                )
