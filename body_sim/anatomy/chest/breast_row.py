...
class BreastRow(AnatomicalComponent):
    ...
    def _emit(self, event_name: str, event_type: Optional[EventType] = None, **data: Any) -> None:
    ...
    def add_fluid_by_type(self, fluid_type: FluidType, amount: float, source: str = None) -> float:
        ...
        if self.filled >= self.GIGA_LIMIT:
            self._emit("max_capacity_reached", EventType.MODIFIER_APPLIED, limit=self.GIGA_LIMIT)
            return 0.0
        ...
        if amount <= to_full:
            self.mixture.add(fluid_type, amount, source or getattr(self, 'component_id', 'unknown'))
            self._emit("fluid_added", EventType.FLUID_ADDED, 
                      amount=amount, fluid_type=fluid_type.name,
                      inflated=False)
            return amount
        ...
        if self.fill_ratio >= 0.90 and self._auto_inflate:
            ...
            self._emit("fluid_added", EventType.FLUID_ADDED, 
                      amount=amount, fluid_type=fluid_type.name,
                      inflated=True, stretch_before=old_stretch, 
                      stretch_after=self.inflation.profile.stretch_ratio)
            return amount
        
        # Если не растягиваемся - добавляем только до полного заполнения
        if to_full > 0:
            self.mixture.add(fluid_type, to_full, source or getattr(self, 'component_id', 'unknown'))
            self._emit("fluid_added", EventType.FLUID_ADDED, 
                      amount=to_full, fluid_type=fluid_type.name,
                      inflated=False, note="partial_fill")
            return to_full
    ...
    def express(self, amount: float, pressure: float = 1.0) -> float:
        ...
        if total > 0:
            ...
            self._emit("express", EventType.FLUID_REMOVED, amount=total, fluid_type="MILK")
    ...
    def stimulate(self, intensity: float = 0.1) -> dict:
        ...
        self._emit("stimulated", EventType.MODIFIER_APPLIED, 
                  intensity=intensity, arousal=self.arousal, 
                  pleasure=self.pleasure, side=self.side)
    ...
    def start_lactation(self, intensity: float = 10.0) -> None:
        try:
            ...
            self._emit("lactation_start", EventType.LACTATION_START, intensity=intensity)
        except:
    ...
    def _calc_leak_rate(self, pressure: float) -> float:
        ...
        open_nipples = [n for n in self.areola.nipples if n.is_open and n.effective_gape > 0]
        if not open_nipples:
            return 0.0
        
        viscosity = self.mixture.viscosity() if hasattr(self.mixture, 'viscosity') else 1.0
        total_flow = 0.0
    ...
    def _determine_state(self, pressure: float) -> BreastState:
        ...
        if pressure < self.PRESSURE_LEAK_MIN:
            has_open = any(n.is_open for n in self.areola.nipples) if self.areola.nipples else False
            if not has_open:
                return BreastState.OVERPRESSURED
            return BreastState.TENSE  # Если соски открыты но давление ниже порога утечки
    ...
    def tick(self, dt: float = 1.0, event_bus: EventBus = None) -> Dict[str, Any]:
        ...
        # Обновление инфляции
        target_stretch = self.inflation.calculate_target_stretch(self.volume, self._base_volume)
        ...
        try:
            ...
            if lactation_result and lactation_result.get('milk_produced'):
                ...
                self._emit("lactation", EventType.LACTATION_ACTIVE, 
        ...
        if self.filled > 0 and calculated_pressure > 0.3:
            ...
            if leak_rate > 0:
                ...
                if leaked > 0.1:  # Минимальный порог для отображения
                    ...
                    self._emit("leak", EventType.LEAK, 
                              amount=leaked, pressure=calculated_pressure, 
                              side=self.side)
        ...
        if current_state != old_state:
            ...
            self._emit("state_change", EventType.STATE_CHANGE, 
                      old_state=old_state.name, new_state=current_state.name)
        ...
        if current_cup != self._last_current_cup:
            self._emit("cup_changed", EventType.CUP_CHANGE, 
                      old_cup=self._last_current_cup.name, new_cup=current_cup.name)
        ...
        return {
            'state': self._state.name if isinstance(self._state, Enum) else str(self._state),
            'filled': round(self.filled, 1),
            'pressure': round(calculated_pressure, 2),
            'leaked': round(leaked, 2),
            'cup': self.current_cup.name if isinstance(self.current_cup, Enum) else str(self.current_cup),
    ...
    def get_full_state(self) -> Dict[str, Any]:
        ...
        try:
            if hasattr(self.lactation, 'profile') and hasattr(self.lactation.profile, 'state'):
        ...
        return {
            'cup': self.current_cup.name if isinstance(self.current_cup, Enum) else str(self.current_cup),
            'base_cup': self.cup_size.name if isinstance(self.cup_size, Enum) else str(self.cup_size),
            'filled': round(self.filled, 1),
