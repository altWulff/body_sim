...
class Fluid:
    ...
    def merge(self, other: "Fluid") -> "Fluid":
        ...
        # Усредняем свойства
        new_density = (
            self.volume * self.properties.density
            + other.volume * other.properties.density
        ) / total_vol
        new_viscosity = (
            self.volume * self.properties.viscosity
            + other.volume * other.properties.viscosity
        ) / total_vol

        return Fluid(
            fluid_type=(
                self.fluid_type if self.volume > other.volume else other.fluid_type
            ),
            volume=total_vol,
...
class FluidMixture:
    ...
    def add(
        self, fluid_type: FluidType, amount: float, source: str = "unknown"
    ) -> float:
        """Добавить жидкость."""
        if amount <= 0:
            return 0.0
    ...
    def remove(
        self, amount: float, prefer_types: Optional[List[FluidType]] = None
    ) -> Dict[FluidType, float]:
    ...
    def viscosity(self) -> float:
        ...
        visc_sum = sum(
            amount * ftype.viscosity for ftype, amount in self._contents.items()
        )
    ...
    def density(self) -> float:
        ...
        dens_sum = sum(
            amount * ftype.density for ftype, amount in self._contents.items()
        )
    ...
    def to_fluids_list(self, source: str = "mixture") -> List[Fluid]:
        ...
        return [
            Fluid(fluid_type=ftype, volume=vol, source_component=source)
            for ftype, vol in self._contents.items()
            if vol > 0
        ]
...
class FluidContainer:
    ...
    def remove_fluid(self, amount: float, fluid_type: FluidType = None) -> List[Fluid]:
        ...
        return [
            Fluid(
                fluid_type=ftype, volume=vol, source_component=self.__class__.__name__
            )
