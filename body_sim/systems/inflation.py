...
class InflationSystem:
    ...
    def calculate_target_stretch(self, current_volume: float, base_volume: float) -> float:
    ...
    def _calculate_recovery(self) -> float:
        ...
        plastic_part = 1.0 + (self.profile.stretch_ratio - 1.0) * self.profile.plasticity
        elastic_part = (self.profile.stretch_ratio - 1.0) * (1.0 - self.profile.plasticity)
    ...
    def apply_stretch(self, target: float, dt: float = 1.0) -> Dict:
        ...
        self.profile.stretch_ratio = max(1.0, min(
            old + (target - old) * speed * dt,
            self.profile.max_stretch
        ))
    ...
    def _update_elasticity(self):
        ...
        if self.profile.stretch_ratio > 1.0:
            ...
            self.profile.current_elasticity = max(0.1, self.profile.base_elasticity - penalty)
        else:
    ...
    def get_state(self) -> StretchState:
        ...
        if s < 1.1:
            ...
            return StretchState.ELASTIC if not self.profile.is_permanently_stretched else StretchState.PLASTIC
        elif s < self.profile.max_stretch * 0.9:
            return StretchState.PLASTIC if self.profile.is_permanently_stretched else StretchState.ELASTIC
        else:
            return StretchState.DAMAGED if self.profile.stretch_marks > 0.5 else StretchState.PERMANENT
