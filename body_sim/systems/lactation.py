...
class LactationSystem:
    ...
    def stimulate(self, intensity: float = 1.0):
        ...
        self.profile.consecutive_stimulation = min(
            self.profile.max_streak,
            self.profile.consecutive_stimulation + intensity
        )
    ...
    def is_producing(self) -> bool:
        ...
        return self.profile.state in (LactationState.PREPARE, LactationState.ACTIVE, LactationState.ENGORGED)

    def tick(self, delta_time: float, breast: 'BreastComponent', event_bus: EventBus) -> dict:
        """Обновление."""
        ...
        if self.profile.state == LactationState.PREPARE:
            ...
            produced = self.calculate_rate(breast._base_volume, fill_ratio) * 0.5 * delta_time
