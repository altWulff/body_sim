...
class Body:
    ...
    def transform_clitoris_to_penis(
        self,
        clitoris_idx: int = 0,
    ...
    def ejaculate(self, penis_index: int = 0) -> Dict[str, Any]:
        if not self.reproductive_system or penis_index >= len(
            self.reproductive_system.penises
        ):
    ...
    def inflate_uterus(self, uterus_idx: int = 0, ratio: float = 2.0):
        if not self.reproductive_system or uterus_idx >= len(
            self.reproductive_system.uteri
        ):
            return False
    ...
    def get_full_state(self) -> Dict[str, Any]:
        return {
            ...
            "digestive": (
                self.digestive_system.get_state() if self.digestive_system else None
            ),
            "reproductive": (
                self.reproductive_system.get_state()
                if self.reproductive_system
                else None
            ),
