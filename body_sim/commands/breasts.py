...
def register_breast_commands(registry: CommandRegistry):
    ...
    def cmd_breasts_show(ctx: CommandContext, side: str = "left"):
        ...
        if side.lower() in ("both", "b"):
            ...
            left_state = (
                left.state.name if hasattr(left.state, "name") else str(left.state)
            )
            right_state = (
                right.state.name if hasattr(right.state, "name") else str(right.state)
            )

            left_lact = "Unknown"
            right_lact = "Unknown"
            try:
                if hasattr(left.lactation, "profile") and hasattr(
                    left.lactation.profile, "state"
                ):
                    left_lact = (
                        left.lactation.profile.state.name
                        if hasattr(left.lactation.profile.state, "name")
                        else str(left.lactation.profile.state)
                    )
                if hasattr(right.lactation, "profile") and hasattr(
                    right.lactation.profile, "state"
                ):
                    right_lact = (
                        right.lactation.profile.state.name
                        if hasattr(right.lactation.profile.state, "name")
                        else str(right.lactation.profile.state)
                    )
            except:
                pass
            ...
            table.add_row(
                "Stretch",
                f"{left.inflation.profile.stretch_ratio:.2f}x",
                f"{right.inflation.profile.stretch_ratio:.2f}x",
            )
            ...
            state_str = (
                breast.state.name
                if hasattr(breast.state, "name")
                else str(breast.state)
            )

            table.add_row("Base Cup", breast.cup_size.name)
            table.add_row(
                "Volume", f"{breast.volume:.1f}ml"
            )  # ИСПРАВЛЕНО: убрано current_volume
            table.add_row(
                "Filled", f"{breast.filled:.1f}ml / {breast._max_volume:.1f}ml"
            )
            ...
            lact_state = "Unknown"
            try:
                if hasattr(breast.lactation, "profile") and hasattr(
                    breast.lactation.profile, "state"
                ):
                    lact_state = (
                        breast.lactation.profile.state.name
                        if hasattr(breast.lactation.profile.state, "name")
                        else str(breast.lactation.profile.state)
                    )
            except:
                pass
            ...
            if hasattr(breast.lactation, "_total_produced"):
                try:
                    table.add_row(
                        "Milk Produced", f"{breast.lactation._total_produced:.1f}ml"
                    )
                except:
                    pass
            ...
            if hasattr(breast.inflation, "profile"):
                table.add_row(
                    "Stretch Ratio", f"{breast.inflation.profile.stretch_ratio:.2f}x"
                )
                if hasattr(breast.inflation, "get_skin_tension"):
                    table.add_row(
                        "Skin Tension", f"{breast.inflation.get_skin_tension():.1%}"
                    )
                if hasattr(breast.inflation.profile, "stretch_marks"):
                    table.add_row(
                        "Stretch Marks", f"{breast.inflation.profile.stretch_marks:.1%}"
                    )
    ...
    def cmd_breasts_add(
        ctx: CommandContext, amount: str, fluid_type: str = "milk", side: str = "left"
    ):
        ...
        try:
            ...
            ctx.console.print(
                f"[yellow]Unknown fluid type {fluid_type}, using MILK[/yellow]"
            )
        ...
        if actual > 0:
            ctx.console.print(
                f"[green]Added {actual:.1f}ml {ftype.name} to {side} breast[/green]"
            )
            if breast.inflation.profile.stretch_ratio > 1.1:
                ctx.console.print(
                    f"[dim]Stretch: {breast.inflation.profile.stretch_ratio:.2f}x[/dim]"
                )
        else:
            ctx.console.print(f"[red]Could not add fluid (full or error)[/red]")

    def cmd_breasts_express(
        ctx: CommandContext, amount: str = "50", side: str = "left"
    ):
        ...
        if removed > 0:
            ctx.console.print(
                f"[green]Expressed {removed:.1f}ml from {side} breast[/green]"
            )
            # Стимуляция лактации
            try:
    ...
    def cmd_breasts_lactation(
        ctx: CommandContext, action: str = "start", side: str = "left"
    ):
        ...
        if side.lower() in ("both", "b"):
            ...
            if action.lower() == "start":
                ...
                ctx.console.print(
                    f"[yellow]Lactation stopped for {side} breast[/yellow]"
                )
            else:
                ctx.console.print(
                    f"[red]Unknown action: {action} (use start/stop)[/red]"
                )

    def cmd_breasts_stimulate(
        ctx: CommandContext, intensity: str = "1.0", side: str = "left"
    ):
        ...
        if side.lower() in ("both", "b"):
            ...
            ctx.console.print(
                f"[green]Stimulated both breasts (intensity: {intensity})[/green]"
            )
            total = (
                l_pleasure.get("pleasure", 0) if isinstance(l_pleasure, dict) else 0
    ...
    def cmd_breasts_tick(ctx: CommandContext, dt: str = "1.0", side: str = "left"):
        ...
        if side.lower() in ("both", "b"):
            ...
            if left:
                ...
                ctx.console.print(
                    f"[dim]Left: state={result.get('state')}, filled={result.get('filled', 0):.1f}ml[/dim]"
                )
            if right:
                result = right.tick(float(dt), ctx.body.event_bus)
                ctx.console.print(
                    f"[dim]Right: state={result.get('state')}, filled={result.get('filled', 0):.1f}ml[/dim]"
                )
        else:
            breast = get_breast(ctx, side)
            if not breast:
    ...
    def cmd_breasts_insert(
        ctx: CommandContext, object_name: str, volume: str, side: str = "left"
    ):
        ...
        if breast.insert_object(obj_data):
            ctx.console.print(
                f"[green]Inserted {object_name} ({volume}ml) into {side} breast[/green]"
            )
            # Давление обновляется автоматически в tick
        else:
            ctx.console.print(f"[red]Cannot insert (too large or blocked)[/red]")

    def cmd_breasts_remove_object(
        ctx: CommandContext, object_name: str, side: str = "left"
    ):
        ...
        if obj:
            ctx.console.print(
                f"[green]Removed {obj.get('name', object_name)} from {side} breast[/green]"
            )
        else:
            ctx.console.print(f"[yellow]Object {object_name} not found[/yellow]")

    def cmd_breasts_nipple(
        ctx: CommandContext,
        action: str = "open",
        amount: str = "0.5",
        side: str = "left",
        ...
        if action.lower() == "open":
            ...
            ctx.console.print(
                f"[green]Nipple opened to {nipple.gape_diameter:.2f}cm[/green]"
            )
    ...
    def cmd_breasts_fullness(ctx: CommandContext, side: str = "left"):
        ...
        if side.lower() in ("both", "b"):
            ...
            if (
                left.state.name == "OVERPRESSURED"
                or right.state.name == "OVERPRESSURED"
            ):
    ...
    registry.register(
        "breasts.show",
        cmd_breasts_show,
        "Show breast status [left/right/both]",
        aliases=["bs"],
        category="Грудь",
    )
    registry.register(
    ...
    registry.register(
        "breasts.express",
        cmd_breasts_express,
        "Express milk [amount] [side]",
        aliases=["be"],
        category="Грудь",
    )
    registry.register(
        "breasts.lactation",
        cmd_breasts_lactation,
        "Lactation control: <start/stop> [side]",
        aliases=["bl"],
        category="Грудь",
    )
    registry.register(
        "breasts.stimulate",
        cmd_breasts_stimulate,
        "Stimulate [intensity] [side]",
        aliases=["bst"],
        category="Грудь",
    )
    registry.register(
    ...
    registry.register(
        "breasts.insert",
        cmd_breasts_insert,
        "Insert object: <name> <volume> [side]",
        aliases=["bi"],
        category="Грудь",
    )
    registry.register(
        "breasts.remove_object",
        cmd_breasts_remove_object,
        "Remove object: <name> [side]",
        aliases=["bro"],
        category="Грудь",
    )
    registry.register(
        "breasts.nipple",
        cmd_breasts_nipple,
        "Nipple control: <open/close> [amount] [side]",
        aliases=["bn"],
        category="Грудь",
    )
    registry.register(
        "breasts.fullness",
        cmd_breasts_fullness,
        "Показать заполненность груди [left/right/both]",
        aliases=["bf", "bfullness"],
        category="Грудь",
    )
